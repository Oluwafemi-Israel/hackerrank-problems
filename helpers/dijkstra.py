...
def dijkstra(ref_node):
	...
	closest_adjacent_node = min(distance_values_set - ref_node.shortest_path_set, key=lambda adjacent_node: adjacent_node[1])
	...
	# update distance values of nodes ajdacent to the recently found closest_adjacent_node
	for key, value in closest_adjacent_node[0].distance_values.items():	
		...
		if adjacent_node_to_closest_node[0] in dict(ref_node.distance_values):
			...
			if (ref_node.distance_values[closest_adjacent_node[0]] + adjacent_node_to_closest_node[1]) < ref_node.distance_values[adjacent_node_to_closest_node[0]]:
				ref_node.distance_values[adjacent_node_to_closest_node[0]] = ref_node.distance_values[closest_adjacent_node[0]] + adjacent_node_to_closest_node[1]
